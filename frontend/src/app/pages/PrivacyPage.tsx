...
function AddSlotModal({
  ...
  const handleSubmit = () => {
    ...
    onAdd({
      ...
      isDefault: false,
      isActive: true,
    } as Omit<ScheduleSlot, "id"> & { isActive: boolean });
    onClose();
  };
    ...
    >
      ...
      >
        ...
        <p className="text-xs text-gray-500">
          Override your default schedule for a specific day and time window —
          great for events or late nights.
        </p>
        ...
        <button
          onClick={() => {
            onAdd({ date, startTime, endTime, mode, note });
            onClose();
          }}
...
export function PrivacyPage() {
  ...
  const [activeSection, setActiveSection] = useState<
    "schedule" | "exceptions" | "precision" | "zones"
  >("schedule");
    ...
    <div className="h-full flex flex-col bg-gray-50">
        ...
        <div className="bg-gray-50 rounded-2xl p-4">
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              <span
                className={`text-[10px] ${isSharing ? "text-green-100" : "text-gray-400"}`}
              >
                Friends can see you
              </span>
            </button>
            ...
            >
              ...
              <span
                className={`text-[10px] ${!isSharing ? "text-amber-100" : "text-gray-400"}`}
              >
                Location hidden
              </span>
            </button>
          ...
          <p className="text-[10px] text-gray-400 text-center mt-2">
            This overrides your schedule. Your schedule will resume at the next
            scheduled time.
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        ...
        {activeSection === "schedule" && (
          <>
            <ScheduleBar
              slots={currentUser.scheduleSlots.filter((s) => s.isActive)}
            />
            ...
            <div className="space-y-2">
              {currentUser.scheduleSlots.map((slot) => (
                <SlotCard
                  ...
                  onToggle={() =>
                    updateScheduleSlot(slot.id, { isActive: !slot.isActive })
                  }
                />
              ))}
            </div>
            ...
            <div className="bg-blue-50 rounded-2xl p-4 border border-blue-100 flex gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-600 mt-0.5">
                  Schedules run automatically every day. Private slots override
                  sharing ones. You can always manually override using the
                  toggle above.
                </p>
              </div>
            </div>
          ...
          <>
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100 flex gap-3">
              ...
              <p className="text-xs text-amber-700">
                Exceptions override your default schedule for a specific date
                and time window — great for events or late nights.
              </p>
            </div>
            ...
            {currentUser.exceptions.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Add one-off overrides for specific dates
                </p>
              </div>
            ) : (
              <div className="space-y-2">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <div>
                    <p
                      className={`text-sm font-semibold ${currentUser.locationMode === "exact" ? "text-indigo-700" : "text-gray-700"}`}
                    >
                      Exact Location
                    </p>
                    <p className="text-xs text-gray-500 mt-0.5">
                      Friends see your precise pin on the map. Best for meetups
                      and coordination.
                    </p>
                  </div>
                  {currentUser.locationMode === "exact" && (
                ...
                >
                  ...
                  <div>
                    <p
                      className={`text-sm font-semibold ${currentUser.locationMode === "binary" ? "text-purple-700" : "text-gray-700"}`}
                    >
                      Zone Only (Binary)
                    </p>
                    <p className="text-xs text-gray-500 mt-0.5">
                      Friends only see whether you're inside or outside a
                      geofence — not your exact position.
                    </p>
                  </div>
                  {currentUser.locationMode === "binary" && (
            ...
            <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-700 font-medium">
                  {currentUser.locationMode === "exact"
                    ? "Sharing exact coordinates"
                    : "Binary zone mode active"}
                </span>
              </div>
            </div>
          </>
          ...
          <>
            <div className="bg-indigo-50 rounded-2xl p-4 border border-indigo-100">
              ...
              <p className="text-xs text-indigo-600">
                Your location is only shared when you are inside an active
                geofence. Outside all zones, you appear as "Not sharing."
              </p>
            </div>

            <div className="space-y-2">
              {geofences.map((fence) => {
                const isActive = currentUser.activeGeofenceIds.includes(
                  fence.id
                );
                return (
                  <div
      ...
      <AnimatePresence>
        {showAddSlot && (
          <AddSlotModal
            onAdd={(slot) =>
              addScheduleSlot(slot as Parameters<typeof addScheduleSlot>[0])
            }
