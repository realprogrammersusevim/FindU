          ...
          {!loading &&
            results.map((user) => (
              ...
              >
                ...
                {sent.has(user.id) ? (
                  <span className="text-xs font-semibold text-green-600 px-3 py-1.5 bg-green-50 rounded-xl">
                    Sent ✓
                  </span>
                ) : (
                  <button
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1 mt-0.5">
            <span
              className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${statusConfig.bg} ${statusConfig.color}`}
            >
              {statusConfig.label}
            </span>
            ...
            <div className="px-4 pb-4 pt-0 border-t border-gray-50">
              <div className="pt-3 space-y-3">
                  ...
                  <div className="bg-indigo-50 rounded-xl p-3">
                    ...
                    {friend.location.mode === "exact" ? (
                      <p className="text-xs text-indigo-600">
                        Exact position shared · Updated{" "}
                        {friend.location.lastUpdated}
                      </p>
                    ) : (
                      <p className="text-xs text-indigo-600">
                    ...
                    <div className="flex gap-1 mt-2 flex-wrap">
                      {friend.location.withinFences.map((fid) => (
                        <span
                          key={fid}
                          className="text-[10px] bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full font-medium"
                        >
                ...
                <div className="flex gap-2">
                  ...
                  >
                    <Star
                      className={`w-3.5 h-3.5 ${friend.isFavorite ? "fill-amber-500" : ""}`}
                    />
...
export function FriendsPage() {
  ...
  const handleViewOnMap = (friend: Friend) => {
    ...
    navigate("/map", {
      state: { centerOn: friend.location.position, focusFriendId: friend.id },
    });
  };
